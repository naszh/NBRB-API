...
function getOnDate(curr, fromDate) {
	curr.currs.map(el => {
		if (
			new Date(fromDate.value).getTime() <= new Date(el.dateE).getTime() &&
...
function splitLinks(curr, fromDate, toDate) {
	...
	for (let i = 0; i < curr.length; i++) {
		...
		while (
			nextYear < endYearCurr ||
			(nextYear === endYearCurr &&
				new Date(`${nextYear}-${dateWithoutYear}`).getTime() <
					new Date(curr[i].dateE).getTime())
		) {
			if (
				nextYear > endYear ||
				...
				links.push(
					`${baseUrl}/rates/dynamics/${curr[i].id}?startdate=${startYear}-${dateWithoutYear}&enddate=${toDate.value}`
				);
				return links;
			}
			links.push(
				`${baseUrl}/rates/dynamics/${curr[i].id}?startdate=${startYear}-${dateWithoutYear}&enddate=${nextYear}-${dateWithoutYear}`
			);
		...
		if (
			(startYear === endYearCurr &&
				dateWithoutYear < curr[i].dateE.slice(5, 10)) ||
			(startYear < endYearCurr &&
				new Date(`${nextYear}-${dateWithoutYear}`).getTime() >
					new Date(curr[i].dateE).getTime())
		) {
			links.push(
				`${baseUrl}/rates/dynamics/${
					curr[i].id
				}?startdate=${startYear}-${dateWithoutYear}&enddate=${curr[
					i
				].dateE.slice(0, 10)}`
			);
